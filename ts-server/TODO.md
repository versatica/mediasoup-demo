# TODO

- Run a timer in case the peer doesn't send "Join" in N seconds.
- Don't make client assume that HTTPS listen port is 4443. Instead make client app read `config.mjs` somehow.
- Edit `.gitignore` in root folder.
- In README say that env to start interactive terminal is now "TERMINAL=true" instead of "INTERACTIVE=true".
- Throttle stuff.
- Logging of `worker.getResourceUsage()` stuff.
- `Room.ts`: `activeSpeakerObserver` and `audioLevelObserver` stuff.
- CI stuff.
- `npm run release` stuff.
- Tons of changes in WS messages (requests moved to notifications, changes in the payloads, etc).
